...
/**
 * 一个对象将汉字转换为对应的拼音字符串。的字符 多个可能的拼音字符串,根据排序器只有一个选择。多音字母不是 支持在这个实现。这个类是实现最佳运行时实现的 与容忍牺牲性能和最小运行时资源的准确性。这
 * 实现高度依赖zh_CN ICU排序数据,必须始终同步 ICU。 < p > 目前这个文件是zh型一致。txt在ICU 4.6
 */
public class HanziToPinyin {
    /**
     * Unihans数组。每个unihans是第一个在相同的拼音。用它来确定拼音 ~ 20 k unihans。
     */
    ...
    public static class Token {
        ...
        /**
         * 翻译的源字符串。汉族,目标是对应的拼音。否则,目标是 来源的原始字符串。
         */
...
//            return sInstance;
    ...
    /**
     * 将输入转换成数组的令牌。ASCII或未知字符的序列 空间将变成一个令牌,一个汉字字符的拼音将被视为一个 令牌。如果这些是中国没有排序器,返回空的标记数组。
     */
