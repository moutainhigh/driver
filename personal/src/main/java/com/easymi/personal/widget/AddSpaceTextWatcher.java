...
/**
 * Created by developerLzh on 2017/6/7.
 */
...
public class AddSpaceTextWatcher implements TextWatcher {
    ...
    private onBinChangedListener binChangedListener;
    public AddSpaceTextWatcher(Context context, EditText editText, int maxLenght) {
    ...
    public void setBinChangedListener(onBinChangedListener binChangedListener) {
        this.binChangedListener = binChangedListener;
    }
    @Override
    ...
    public void afterTextChanged(Editable s) {
        if (isChanged) {
            ...
            if (binChangedListener != null && editStr.length() == 7) {//拿到binNumber
                editStr = editStr.replace(" ", "");
                Long lo = Long.parseLong(editStr);
                binChangedListener.onChanged(AssetsBankInfo.getNameOfBank(context, lo));
            }
        }
    }
    /**
    ...
    public interface onBinChangedListener {
        void onChanged(String bankName);
    }
}